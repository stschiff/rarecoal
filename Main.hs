...
parseMcmcOpt = McmcOpt <$> parseTheta <*> parseTemplateFilePath <*> parseParams
  ...
  where
    ...
    parseNrBurninCycles = OP.option OP.auto $ OP.long "burnin" <> OP.short 'b' <> OP.metavar "<INT>"
                                                               <> OP.help "nr of burnin cycles"
    parseNrMainCycles = OP.option OP.auto $ OP.long "cycles" <> OP.short 'c' <> OP.metavar "<INT>"
