...
parseNrCalledSites = OP.option OP.auto $ OP.short 'N' <> OP.long "nr_called_sites"
                                                      <> OP.metavar "INT"
                                                      <> OP.value 0
                                                      <> OP.help "set the nr of called sites. This will affect only the number of non-variant sites, mainly added for legacy purposes for histograms without a non-variant pattern."
...
parseProbOpt = ProbOpt <$> parseTheta <*> parseTemplateFilePath <*> parseInitialParamsFile
                       ...
                       <*> OP.argument OP.auto (OP.metavar "NVec" <> OP.help "number of samples, comma-separated, without spaces, and surrounded by square brackets, e.g. [100,100] for two populations with a 100 haploid samples each")
                       <*> OP.argument OP.auto (OP.metavar "MVec" <> OP.help "number of derived alleles in each population, same format as for NVec, e.g. [1,2] for allele count 3 shared with one sample from the first and two from the second population.")

parseTheta :: OP.Parser Double
parseTheta = OP.option OP.auto $ OP.short 't' <> OP.long "theta"
                                              <> OP.metavar "DOUBLE"
                                              <> OP.value 0.0005 <> OP.showDefault
                                              <> OP.help "set the scaled mutation rate. This is only used for scaling and should rarely be changed from the default."

parseTemplateFilePath :: OP.Parser FilePath
parseTemplateFilePath = OP.strOption $ OP.short 'T' <> OP.long "template" <>
                                       OP.metavar "<Input Template File>" <> OP.value "/dev/null" <> 
...
parseInitialParamsList = OP.option OP.auto $ OP.short 'x' <> OP.long "params" <>
                         ...
                         OP.help "initial parameters for the template, given as comma-separated list without spaces and surrounded by square-brackets, e.g. [1.0,1.0,0.1,0,1] for four parameters. The number given here must match the number of parameters in the model template file."
...
parseInitialParamsFile = OP.strOption $ OP.long "paramsFile" <>
                         ...
                         OP.help "read the initial parameters from a file, which can be the main output file from either the maxl or the mcmc commands. This is useful for continuing an optimization from a previous run, or for starting an MCMC run from the maximum found by maxl"
...
parseJoin = OP.option (OP.str >>= readJoin) $ OP.short 'j' <> OP.long "join"
                                              <> OP.metavar "t,k,l"
                                              <> OP.help "Specify a join event at time t from l into k. Can be given multiple times. Ignored if a model template file is given. Example: -j 0.1,0,1 specifies a join from branch 1 into branch 0 at time 0.1.  You can give this option multiple times"
  where
    readJoin s = do
        let [t, k, l] = splitOn "," s
...
parseSetP = OP.option (OP.str >>= readSetP) $ OP.short 'p' <> OP.long "popSize"
                                              <> OP.metavar "t,k,p"
                                              <> OP.help "Specify a population size change event at time t in branch k to population size p. Can be given multiple times. Ignored if a model template file is given. Example: -p 0.1,0,2.2 specifies that at time 0.1 the population size in branch 0 should be set to 2.2. You can give this option multiple times"
  where
    readSetP s = do
        let [t, k, p] = splitOn "," s
...
parseLinGen = OP.option OP.auto $ OP.long "lingen"
                                              ...
                                              <> OP.help "set the number of approximately linearly spaced time intervals in the  discretization. This is described in doc/rarecoal.pdf. You can use this parameter to speed up the computation at the cost of accuracy. For example, set this to 50 and everything will run much faster but be less correct. It is useful for example to quickly search for a rough estimate of a maximum using maxl, and then rerunning with the finer standard discretization (400)."
...
parseMaxlOpt = MaxlOpt <$> parseTheta <*> parseTemplateFilePath <*> parseInitialParamsFile
  ...
  where
    ...
    parseNrRestarts = OP.option OP.auto $ OP.long "nrRestarts"
                                                      ...
                                                      <> OP.help "Specifies the number of restarts of the minimization routine. Each restart will start from the end point of the previous maximization. From experience, five restarts are typically enough to get close to the true maximum."

parseTraceFilePath :: OP.Parser FilePath
parseTraceFilePath = OP.strOption $ OP.short 'f' <> OP.long "traceFile" <> OP.metavar "<FILE>"
                                                            <> OP.value "/dev/null"
                                                            <> OP.help "The file to write the trace of the maximization or the MCMC. Can be useful to check whether parameters are converging."
...
parseMcmcOpt = McmcOpt <$> parseTheta <*> parseTemplateFilePath <*> parseInitialParamsFile <*> parseInitialParamsList
  ...
  where
    ...
    parseNrCycles = OP.option OP.auto $ OP.long "cycles" <> OP.short 'c' <> OP.value 1000 <> OP.metavar "<INT>"
                                                               <> OP.help "nr of MCMC cycles after Burnin. The burnin phase is determined automatically by requiring the likelihood to still systematicallyl increase. Once the likelihood stabilizes, the number of cycles given here are performed. Note that a cycle here means that every parameter is tried." <> OP.showDefault
    parseBranchAges = OP.option OP.auto $ OP.long "branchAges" <> OP.short 'b' <> OP.metavar "<LIST>"
                                                               <> OP.help "scaled ages of samples, to input ancient samples. This should be a comma-separated list without spaces and surrounded by square-brackets. The length of the list must equal the number of populations in the histogram file." <> OP.value []

parseConditioning :: OP.Parser [Int]
parseConditioning = OP.option OP.auto $ OP.long "conditionOn" <> OP.metavar "<List>"
                                        <> OP.help "a comma-separated list without spaces and surrounded by square-brackets. Gives all branches (as 0-based indices) in which you require at least one derived allele for the computation of the likelihood. This is useful for mapping ancient samples onto a tree" <> OP.value []
...
parseFindOpt = FindOpt <$> parseQueryIndex <*> parseEvalFile <*> parseBranchAge <*> parseDeltaTime <*> parseMaxTime
  ...
  where
    ...
    parseBranchAge = OP.option OP.auto $ OP.short 'b' <> OP.long "branchAge" <> OP.metavar "<Double>"
                                                       <> OP.help "sampling age of the query sample"
    parseDeltaTime = OP.option OP.auto $ OP.long "deltaTime" <> OP.metavar "<Double>" <> OP.showDefault
                                                      <> OP.help "time between the points" <> OP.value 0.0005
    ...
    parseIgnoreList = OP.option (OP.str >>= readIgnoreList) $ OP.long "exclude" <> OP.metavar "<list of lists>"
                                                            <> OP.help "a comma-separated list of lists, without spaces, with patterns to exclude from the likelihood computation" <> OP.value [] <> OP.internal
