...
import Numeric.GSL.Minimization (minimize, minimizeD, MinimizeMethodD(..), MinimizeMethod(..))
import Core (update)
...
maximizeLikelihood modelTemplate initialParams hist maxCycles = do
    ...
    let initialParams' = replicate (length initialParams) 1.0
        ...
        {-(minResult, trace) = minimizeD VectorBFGS2 1.0e-8 maxCycles 0.01 0.01 func grad initialParams'-}
        (minResult, trace) = minimize NMSimplex2 1.0e-8 maxCycles [0.01 | _ <- initialParams] func initialParams'
  ...
  where
    scaleTraceMatrix factors t = 
        let rows = toRows t
            ...
            factors' = fromList $ [1.0, 1.0, 1.0] ++ factors
